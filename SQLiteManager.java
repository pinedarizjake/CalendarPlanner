...
//        }
    ...
    public void addNoteToDatabase(Reminder reminder)
    {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();

        ContentValues contentValues = new ContentValues();
        contentValues.put(ID_FIELD, reminder.getId());
        contentValues.put(TITLE_FIELD, reminder.getTitle());
        contentValues.put(DESC_FIELD, reminder.getDescription());
        contentValues.put(DELETED_FIELD, getStringFromDate(reminder.getDeleted()));
        ...
        try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME, null))
        {
            if(result.getCount() != 0)
            {
                while (result.moveToNext())
                {
                    ...
                    Reminder note = new Reminder(id,title,desc,deleted);
                    Reminder.reminderArrayList.add(note);
                }
            }
        }
    }

    public void updateNoteInDB(Reminder reminder)
    {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(ID_FIELD, reminder.getId());
